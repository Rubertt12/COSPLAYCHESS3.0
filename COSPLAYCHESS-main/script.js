...
const wallpaperSVGS = {
    naruto: 'img/img/narutoxgoku.png',
    bleach: 'img/img/bleach.png',
    yugioh: 'img/img/Yugioh.png',
    sakuracard: 'img/img/sakuracard.png',
    kuroshitsuji: 'img/img/kuroshitsuji.png',
...
function presetDataUrl(key) { 
    ...
    if (!val) return '';
    // Se for um link direto ou caminho de imagem local, retorna o link direto.
    if (typeof val === 'string' && (val.startsWith('http') || val.match(/\.(png|jpe?g|webp|gif)$/i) || val.startsWith('img/'))) {
        return val;
    }
    return 'data:image/svg+xml,' + encodeURIComponent(val);
}
...
function revertTheme() {
    applyTheme(store.g.theme || 'default');
}
...
function applyFullscreen(enabled) {
    store.g.fullscreen = !!enabled;
    if (window.electronAPI?.setFullscreen) {
        window.electronAPI.setFullscreen(store.g.fullscreen).catch(() => {});
    } else if (document.documentElement.requestFullscreen) {
        if (store.g.fullscreen && !document.fullscreenElement) {
            document.documentElement.requestFullscreen().catch(() => {});
        } else if (!store.g.fullscreen && document.fullscreenElement) {
...
function toggleFullscreen(enabled) {
    ...
    if (document.getElementById('fullscreen-setting')) {
        document.getElementById('fullscreen-setting').checked = !!enabled;
    }
}
...
function upWallpaper(input) {
    ...
    reader.onload = function(e) {
        ...
        if (file.type.startsWith('video/')) {
            ...
            let video = document.getElementById('custom-wallpaper-video');

            if (!video) {
                video = document.createElement('video');
                video.id = 'custom-wallpaper-video';
    ...
    store.board.forEach((id, i) => {
        ...
        if(id) {
            ...
            if (store.p[id]?.img) {
                ...
                const type = id.charAt(0);
                const isWhite = id.endsWith('_B');
                // Usa glifos Unicode para as peças de xadrez
                const glyphs = {
                    'P': { w: '♙', b: '♟' },
                    'T': { w: '♖', b: '♜' },
                    'C': { w: '♘', b: '♞' },
                ...
                const glyph = (glyphs[type] && (isWhite ? glyphs[type].w : glyphs[type].b)) || '?';
                
                p.style.backgroundColor = isWhite ? '#fff' : '#000'; 
                p.style.color = isWhite ? '#000' : '#fff';
                p.innerText = glyph;
            }
...
function handleSq(i) {
    ...
    if (store.g.mode === 'AI' && isLive && turn === store.g.aiSide) return;
...
function openArena() {
    ...
    const setFighter = (el, id, type) => {
        ...
        if (store.p[id]?.img) {
            ...
            // Caso contrário, usa um glifo Unicode de xadrez
            const isWhite = id.endsWith('_B');
            const glyphs = {
                'P': { w: '♙', b: '♟' }, 'T': { w: '♖', b: '♜' },
                'C': { w: '♘', b: '♞' }, 'B': { w: '♗', b: '♝' },
            ...
            el.style.fontSize = 'clamp(24px, 6.5vw, 64px)'; 
            el.style.lineHeight = '1';
            el.style.backgroundColor = isWhite ? '#ffffff' : '#000000';
...
function openStartMenuSettings() {
    playUISound('click');
    document.getElementById('main-start-options').classList.remove('menu-panel-visible');
    document.getElementById('main-start-options').classList.add('menu-panel-hidden');
    document.getElementById('start-menu-settings-content').classList.remove('menu-panel-hidden');
    document.getElementById('start-menu-settings-content').classList.add('menu-panel-visible');
}

function closeStartMenuSettings() {
    playUISound('click');
    document.getElementById('main-start-options').classList.remove('menu-panel-hidden');
    document.getElementById('main-start-options').classList.add('menu-panel-visible');
    document.getElementById('start-menu-settings-content').classList.remove('menu-panel-visible');
