/* Cosplay Chess - posicionamento manual de peças no tabuleiro (Windows/Electron) */
(() => {
  ...
  function pieceName(id) {
    try {
      return String(store?.p?.[id]?.name || pieceNames?.[String(id || '').charAt(0)] || id || 'PEÇA');
    } catch (_) {
      return String(id || 'PEÇA');
    }
  }
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .piece-board-position-btn{width:100%;margin-top:6px!important;border:1px solid rgba(0,229,255,.28)!important;background:rgba(0,229,255,.06)!important;color:#aefaff!important;border-radius:7px!important;padding:8px!important;font-size:8px!important;font-weight:900!important;cursor:pointer!important}
    `;
  ...
  function commitPlacement(targetIndex) {
    ...
    if (sourceIndex >= 0) {
      // Se o destino estiver ocupado, troca as duas peças de lugar sem excluir ninguém.
      store.board[sourceIndex] = targetPiece;
      store.board[targetIndex] = id;
    } else {
      // Peça fora do tabuleiro: só entra em casa vazia para não apagar outra peça.
      if (targetPiece) {
        showBanner(`A casa está ocupada por ${pieceName(targetPiece)}. Escolha uma casa vazia para colocar ${pieceName(id)}. ESC cancela.`);
        return true;
      }
      store.board[targetIndex] = id;
    }

    if (Array.isArray(store.graveyard)) {
      store.graveyard = store.graveyard.filter(piece => piece !== id);
    }
  ...
  function bindBoard() {
    ...
    board.addEventListener('click', event => {
      ...
      const index = Array.prototype.indexOf.call(board.children, sq);
  ...
  function extractId(button) {
    ...
    const match = code.match(/editParticipantDisplayName\(['\"]([^'\"]+)['\"]\)/);
    return match?.[1] || '';
  }
  ...
  function decorateCards() {
    ['list-white', 'list-black'].forEach(listId => {
      ...
      if (!list) return;
      list.querySelectorAll('.participant-audio-card').forEach(card => {
        if (card.querySelector('.piece-board-position-btn')) return;
        const nameButton = card.querySelector('button[onclick*="editParticipantDisplayName"]');
        const id = extractId(nameButton);
        if (!id) return;
        const button = document.createElement('button');
        button.type = 'button';
        button.className = 'piece-board-position-btn';
        button.textContent = '♟ POSICIONAR PEÇA NO TABULEIRO';
        button.title = `Escolher manualmente onde ${pieceName(id)} ficará no tabuleiro`;
