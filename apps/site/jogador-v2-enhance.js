(() => {
  ...
  async function init(){
    ...
    if (aboutPanel) {
      ...
      story.querySelector('p').textContent = bio || 'Este participante ainda não escreveu o Sobre mim.';
      if (!bio) story.querySelector('p').className='empty-about';
      ...
      if (interests) {
        const interestTitle=document.createElement('div');interestTitle.className='section-heading';interestTitle.style.marginTop='22px';interestTitle.innerHTML='<span>GOSTOS & REFERÊNCIAS</span><h2>Interesses</h2>';
        aboutPanel.insertBefore(wrap, interests);
        aboutPanel.insertBefore(interestTitle, interests);
      } else if (heading) heading.insertAdjacentElement('afterend',wrap);
    }
    ...
    if (sideCard) {
      ...
      if (dl) {
        const add=(label,value)=>{const d=document.createElement('div');const dt=document.createElement('dt');dt.textContent=label;const dd=document.createElement('dd');dd.textContent=value;d.append(dt,dd);dl.appendChild(d);};
        add('Evento',eventTitle); add('Membro desde',fmt(p.created_at));
      }
    }

    const summary = document.querySelector('.summary-main');
    if (summary && !summary.querySelector('.profile-extra-strip')) {
      const strip=document.createElement('div');strip.className='profile-extra-strip';
      [['Evento',eventTitle],['Membro desde',fmt(p.created_at)],['Personagem',p.character_name||'—'],['Perfil','Público']].forEach(([label,value])=>{const d=document.createElement('div');const s=document.createElement('span');s.textContent=label;const b=document.createElement('b');b.textContent=value;d.append(s,b);strip.appendChild(d);});
