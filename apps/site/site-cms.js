(() => {
  ...
  const cmsDb = window.supabase.createClient(config.supabaseUrl, config.supabaseKey);
  const esc = (v='') => String(v).replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const q = (s) => document.querySelector(s);
  const qa = (s) => [...document.querySelectorAll(s)];
  const setText = (s, value) => { const el=q(s); if(el && value !== undefined) el.textContent=value; };
  const setHtmlTitle = (s, main, accent, br=true) => {
    const el=q(s); if(!el) return;
    const sep=br?'<br>':' ';
    el.innerHTML=`${esc(main||'')}${sep}<i>${esc(accent||'')}</i>`;
  };
  const setLink = (s, text, href) => {
    const el=q(s); if(!el) return;
  ...
  async function getContent(key){
    const {data,error}=await cmsDb.from('cosplay_site_content').select('content').eq('key',key).eq('published',true).maybeSingle();
  ...
  function applyBrand(content){
    if(content.faviconUrl){
      ...
      if(icon) icon.href=content.faviconUrl;
    }
    if(content.brandImageUrl) qa('.brand img').forEach(img=>img.src=content.brandImageUrl);
  }
  ...
  function applyLanding(c){
    ...
    [[c.feature1Title,c.feature1Text],[c.feature2Title,c.feature2Text],[c.feature3Title,c.feature3Text],[c.feature4Title,c.feature4Text]].forEach((pair,i)=>{
      if(!cards[i]) return;
      const title=cards[i].querySelector('h3'), text=cards[i].querySelector('p');
      if(title && pair[0]!==undefined) title.textContent=pair[0];
      if(text && pair[1]!==undefined) text.textContent=pair[1];
    });
    ...
    if(c.instagramUrl){
      qa('[data-fergorverse-instagram], a[href*="instagram.com/fergorverse"]').forEach(a=>a.href=c.instagramUrl);
    }
    if(c.instagramText){
      const btn=q('#universo [data-fergorverse-instagram]'); if(btn) btn.textContent=`📸 ${c.instagramText}`;
      const footer=q('.footer-instagram'); if(footer) footer.textContent='Instagram @fergorverse ↗';
    }
  ...
  function applyRegistration(c){
    ...
    const musicBlock=q('[data-registration-music]');
    if(musicBlock && typeof c.showMusicFields==='boolean') musicBlock.hidden=!c.showMusicFields;
  ...
  async function init(){
    const isRegistration=Boolean(document.getElementById('signupForm'));
    const key=isRegistration?'registration':'landing';
    const content=await getContent(key);
    if(isRegistration) applyRegistration(content); else applyLanding(content);
  }
