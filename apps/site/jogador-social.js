(() => {
  ...
  const loadViewer = async () => {
    ...
    const { data: mine } = await participantDb.from('cosplay_participant_profiles')
      ...
      .eq('user_id', viewerSession.user.id).neq('registration_status', 'cancelled')
  ...
  const setupFriendAction = async () => {
    ...
    actions.querySelector('[data-player-friend-action]')?.remove();
    if (!viewerProfile) {
      const link = document.createElement('a');
      link.className = 'btn dark player-friend-button'; link.href = './participante.html';
      link.dataset.playerFriendAction = '1'; link.textContent = '♟ Entrar para adicionar';
      actions.appendChild(link); return;
    }
    if (viewerProfile.id === profile.id) {
      const link = document.createElement('a');
      link.className = 'btn dark player-friend-button'; link.href = './comunidade.html';
      link.dataset.playerFriendAction = '1'; link.textContent = '✦ Abrir minha comunidade';
      actions.appendChild(link); return;
    }
    ...
    button.type = 'button'; button.className = 'btn gold player-friend-button'; button.dataset.playerFriendAction = '1';
    if (!relation || ['declined','cancelled'].includes(relation.status)) {
      ...
      button.addEventListener('click', async () => {
        ...
        button.classList.add('pending'); button.textContent = 'Solicitação enviada ✓';
      });
    } else if (relation.status === 'accepted') {
      button.className = 'btn dark player-friend-button'; button.textContent = '✓ Vocês são amigos';
      ...
      button.addEventListener('click', () => location.href = './comunidade.html');
    } else if (relation.status === 'blocked') return;
  ...
  const setupStats = async () => {
    ...
    const stats = document.createElement('div'); stats.className = 'player-social-stats';
    stats.append(makeStat(row?.friend_count || 0, 'amigos'), makeStat(row?.post_count || 0, 'posts'), makeStat(row?.photo_count || 0, 'fotos'), makeStat($('playerAchievementCount')?.textContent || 0, 'conquistas'));
    document.querySelector('.player-identity')?.appendChild(stats);
  };

  const createTabs = () => {
    if (!achievements?.parentElement) return null;
    const nav = document.createElement('nav'); nav.className = 'player-social-tabs';
    nav.innerHTML = '<button type="button" class="active" data-player-tab="wall">Mural</button><button type="button" data-player-tab="achievements">Conquistas</button>';
    const wall = document.createElement('section'); wall.className = 'player-social-panel'; wall.dataset.playerSocialPanel = 'wall';
    ...
    achievements.dataset.playerSocialPanel = 'achievements'; achievements.classList.add('player-social-panel'); achievements.hidden = true;
  ...
  const loadPublicPosts = async () => {
    ...
    const { data, error } = await publicDb.from('cosplay_social_posts')
      .select('id,body,image_path,created_at').eq('author_profile_id', profile.id).eq('visibility', 'public')
      .order('created_at', { ascending:false }).limit(30);
    const posts = error ? [] : data || [];
    feed.replaceChildren();
    if (!posts.length) { feed.innerHTML = '<div class="player-social-empty">Nenhuma publicação pública ainda.</div>'; return; }
    for (const post of posts) {
      const card = document.createElement('article'); card.className = 'player-social-post';
      if (post.image_path) {
        const url = await getSignedImage(post.image_path);
        if (url) {
          const imageWrap = document.createElement('div'); imageWrap.className = 'player-social-post-image';
          const img = document.createElement('img'); img.src = url; img.alt = post.body || `Foto de ${displayName(profile)}`; img.loading = 'lazy';
          imageWrap.appendChild(img); card.appendChild(imageWrap);
        }
      }
      const copy = document.createElement('div'); copy.className = 'player-social-post-copy';
      ...
      card.appendChild(copy); feed.appendChild(card);
    }
  };

  const addCommunityCta = () => {
    if (!content || content.querySelector('.player-community-cta')) return;
    const cta = document.createElement('section'); cta.className = 'player-community-cta';
    const copy = document.createElement('div'); const b = document.createElement('b'); b.textContent = 'Comunidade CosplayChess';
    ...
    copy.append(b, span);
    const link = document.createElement('a'); link.className = 'btn gold'; link.href = viewerProfile ? './comunidade.html' : './participante.html';
    link.textContent = viewerProfile ? 'Abrir comunidade' : 'Entrar na minha área'; cta.append(copy, link); content.appendChild(cta);
  };

  const enhance = async () => {
    if (enhanced || !content || content.hidden) return;
    enhanced = true; profile = await loadProfile(); if (!profile) return;
    await loadViewer(); createTabs(); await Promise.all([setupStats(), setupFriendAction(), loadPublicPosts()]); addCommunityCta();
  };
