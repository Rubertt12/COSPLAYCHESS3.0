...
function ringTone(step=0){const ctx=ensureAudio();if(!ctx||ctx.state!=='running')return;const now=ctx.currentTime;const gain=ctx.createGain();const high=ctx.createOscillator();const body=ctx.createOscillator();high.type='sine';body.type='triangle';const base=1180+step*55;high.frequency.setValueAtTime(base,now);high.frequency.exponentialRampToValueAtTime(base*1.42,now+.075);body.frequency.setValueAtTime(base*.52,now);body.frequency.exponentialRampToValueAtTime(base*.72,now+.09);gain.gain.setValueAtTime(.0001,now);gain.gain.exponentialRampToValueAtTime(.11,now+.008);gain.gain.exponentialRampToValueAtTime(.0001,now+.19);high.connect(gain);body.connect(gain);gain.connect(ctx.destination);high.start(now);body.start(now);high.stop(now+.2);body.stop(now+.2)}
function playRingSequence(){ringTimers.forEach(clearTimeout);ringTimers=[0,180,360,540].map((delay,index)=>setTimeout(()=>ringTone(index),delay))}
