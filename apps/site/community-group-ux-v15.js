(() => {
  'use strict';
  if (window.__CC_COMMUNITY_GROUP_UX_V15__) return;
  window.__CC_COMMUNITY_GROUP_UX_V15__ = true;

  const db = window.getCosplayChessParticipantDb ? window.getCosplayChessParticipantDb() : window.COSPLAYCHESS_PARTICIPANT_DB;
  const $ = id => document.getElementById(id);
  const q = (selector, root = document) => root.querySelector(selector);
  const state = { ctx:null, loading:null, joinBusy:false, rulesSource:'', rules:[], rulesObserver:null, rulesTimer:0 };
  ...
  function applyJoinState(ctx) {
    ...
    if (button.classList.contains('cc14-pending') !== pending) button.classList.toggle('cc14-pending',pending);
  ...
  async function syncOwnerRequests(ctx) {
    ...
    if (!ctx || ctx.group.owner_profile_id !== ctx.profile.id) { old?.remove(); return; }
  ...
  async function handleJoin(button) {
    ...
    try {
      ...
      console.error('[CosplayChess community join v15]',error);
  ...
  function parseRules(source) {
    ...
    push();
    return rules.filter(r => r.title || r.body);
  }
  ...
  function openRules() {
    ...
    state.rules.forEach((rule,index) => {
      ...
      const h = document.createElement('h3'); h.textContent=rule.title || `Regra ${index+1}`; copy.appendChild(h);
      String(rule.body || '').split(/\n\s*\n/).map(x=>x.trim()).filter(Boolean).forEach(part => { const p=document.createElement('p'); p.textContent=part.replace(/\n+/g,' '); copy.appendChild(p); });
  ...
  function enhanceRules() {
    ...
    if (!card || q('.cc14-rules-launch',card)) return;
    ...
    state.rules = rules;
    state.rulesObserver?.disconnect();
    ...
    const k=document.createElement('span'); k.className='kicker'; k.textContent='REGRAS';
  ...
  function observeRules() {
    ...
    if (!root) return;
    state.rulesObserver?.disconnect();
    state.rulesObserver = new MutationObserver(mutations => {
      const relevant = mutations.some(m => {
        const target = m.target instanceof Element ? m.target : m.target?.parentElement;
        return target?.id === 'groupV13Rules' || target?.closest?.('#groupV13Rules');
      });
      if (!relevant) return;
      clearTimeout(state.rulesTimer);
      state.rulesTimer = setTimeout(enhanceRules,60);
    });
    state.rulesObserver.observe(root,{childList:true,subtree:true});
  }
  ...
  async function init() {
    observeRules();
    [120,350,800,1600].forEach(ms => setTimeout(enhanceRules,ms));
    try {
      const ctx = await loadContext();
      applyJoinState(ctx);
      await syncOwnerRequests(ctx);
      [300,900,1800].forEach(ms => setTimeout(() => { if (state.ctx) applyJoinState(state.ctx); },ms));
    } catch (error) {
      console.warn('[CosplayChess community UX v15]',error);
    }
  }
